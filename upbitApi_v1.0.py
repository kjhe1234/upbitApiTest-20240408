...
class MainWindow(QMainWindow, form_class):  # 슬롯 클래스
    ...
    def alarmDataCheck(self, trade_price):
        if self.alarmButton.text() == "알람중지":
            sellPrice = float(self.alarm_price1.text())  # 사용자 가 입력한 매도 목표 가격
            buyPrice = float(self.alarm_price2.text())
    ...
    # 현재 코인 가격이 사용자가 설정해 놓은 매도 가격보다 높아지면 매도알람!
            if sellPrice <= trade_price:
                if self.alarmFlag == 0:
                    ...
                    self.telegram_message(f"지정해 놓은 {sellPrice}원 이상 입니다. 매도 하세요!")
                    self.alarmFlag = 1
            ...
            if buyPrice >= trade_price:
                if self.alarmFlag == 0:
                    print("매수가격 도달!! 매수하세요!!")
                    self.telegram_message(f"코인의 현재 가격이 {trade_price}원이 되었습니다!")
                    self.telegram_message(f"지정해 놓은 {buyPrice}원 이하 입니다. 매수 하세요!")
                    self.alarmFlag = 1
